...
var outletsQuery = {
    ...
    grid : {
        ...
        {
            $("#outletsSort-jqGrid").jqGrid({
                datatype   : "json",
                colNames   : ["网点类型", "网点名称", "交易数量"],
                colModel   : [
                    { name : 'outlets', width : 150, align : 'center', sortable : false, hidden: true},
                    { name : 'outletsName', width: 150, align : 'center', sortable : false,resizable:false},
...
//                	window.parent.showLoading();
            ...
            gridSize.action.doResize("#outletsSort-jqGrid"); 
        }
    }
};
...
function showChildGrid(parentRowID, parentRowKey) {
    ...
    var outlet = $("#outletsSort-jqGrid").jqGrid("getRowData", parentRowKey).outlets;
    ...
    $("#" + childGridID).jqGrid({
        url        : "business/statistics/querybyoutlets/main/loadSubBusinessStatistics.action",
        mtype      : "POST",
        datatype   : "json",
        colNames   : ['时间', '业务类型', '业务名称', '数量'],
        colModel   : [
...
$(document).ready(function() {
    ...
    $("#query").on('click', function() {
        ...
        var startDate = getStartDate();
        var endDate = getEndDate();
        outletsQuery.data.bussType=bussType;
        outletsQuery.data.outlet=outlet;
        outletsQuery.data.startDate=startDate;
        outletsQuery.data.endDate=endDate;
        ...
        $('#exportBtn').attr("disabled", true); 
        $('#tableBtnByOutlets').removeClass('disabled');
        $('#barBtnByOutlets').removeClass('disabled');
        $('#pieBtnByOutlets').removeClass('disabled');
        $('#tableBtnByOutlets').addClass('active');
        $('#barBtnByOutlets').removeClass('active');
        $('#pieBtnByOutlets').removeClass('active');
        ...
        $("#outletsSort-jqGrid").jqGrid("setGridParam", {
            url        : 'business/statistics/querybyoutlets/main/loadBusinessStatistics.action',
            mtype      : "POST",
            page:0,
            postData : {
                bussType  : bussType,
    ...
    $("#tableBtnByOutlets").on("click",function(){
    	$(".ui-layout-center").css("overflow","hidden");
    	$(".center-tool-bar").css("width","100%");
    	$("#table-contains").css("display","none");
    	$("#table-contains2").css("display","none");
    ...
    $(".ui-layout-toggler").click(function(){
    	...
    	if($(this).attr("title")=="Open"){
    		...
    		if($("#barBtnByOutlets").hasClass("active")){
    			barOutlets();
    		}
    		if($("#pieBtnByOutlets").hasClass("active")){
    			pieOutlets();
    		}
    	}
