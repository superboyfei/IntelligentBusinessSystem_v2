		...
		onDialogClose :  function(filesSelected, filesQueued, queueLength) {
			...
			if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					toastr.error(this.queueData.errorMsg);
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					//var replaceQueueItem = confirm('The file named "' + file.name + '" is already in the queue.\nDo you want to replace the existing item in the queue?');//modify by chenling
                    var replaceQueueItem = true;//modify by chenling for ie8
					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg = '文件上传失败，请上传 300M 以内的文件';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 为空.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 不是文件接收类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
